...
export default function useTradesmanMessages({
  ...
  const pendingTargetRef = useRef(
    initialCustomerId
      ? { customerId: initialCustomerId, jobId: initialJobId ?? null }
      : null,
  )
  ...
  const refreshInbox = useCallback(async () => {
    ...
    setChats(result.conversations.map(mapApiConversationToChat))
  ...
  const syncConversationDetails = useCallback(async (conversationId) => {
    ...
    try {
      ...
      const mapped = mapApiConversationToChat(conversation)
  ...
  const resolvePendingTarget = useCallback(
    async (conversations) => {
      ...
      const existing = findConversationMatch(conversations, target)
      if (existing?.id) {
      ...
      try {
        const created = await createConversation(target)
        rawConversationsRef.current = [created, ...rawConversationsRef.current]
        setChats((current) => [mapApiConversationToChat(created), ...current])
