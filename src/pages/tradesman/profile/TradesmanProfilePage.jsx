...
export default function TradesmanProfilePage() {
  ...
  const [profile, setProfile] = useState(DEMO_TRADESMAN_PROFILE)
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      ...
      try {
        ...
        if (!cancelled) {
          setProfile(mapApiUserToProfile(user, 'tradesman'))
        }
      } catch (err) {
        if (!cancelled) {
  ...
  const handleUpdateProfile = useCallback(
    async (payload) => {
      const updatedProfile = await submitProfileUpdate(payload, { role: 'tradesman' })
