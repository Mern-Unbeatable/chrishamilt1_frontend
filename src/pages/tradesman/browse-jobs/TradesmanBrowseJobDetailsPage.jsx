...
export default function TradesmanBrowseJobDetailsPage() {
  ...
  const [job, setJob] = useState(
    useApi ? null : demoBrowseJob ? buildDemoJobDetails(demoBrowseJob) : null,
  )
  ...
  useEffect(() => {
    if (!useApi) {
      const demoJob = DEMO_BROWSE_JOBS.find((item) => item.id === jobId)
      setJob(demoJob ? buildDemoJobDetails(demoJob) : null)
      setLoading(false)
      setError('')
      return undefined
    }
    ...
    async function loadJob() {
      ...
      try {
        const data = await fetchPublicJobDetails(jobId, { token: getAccessToken() })
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-2xl border border-[#E5E7EB] bg-white px-6 py-12 text-center">
          ...
          <p className="mt-2 text-sm text-[#64748B]">
            This lead may have been removed or is no longer available.
          </p>
          <Link
            to="/tradesman/browse-jobs"
